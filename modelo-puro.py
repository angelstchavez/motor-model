...
class Simulacion:
    def __init__(self):
        ...
        pygame.display.set_caption("Modelos y Simulación: Robot Diferencial")
    ...
    def dibujar_rectangulo_rotado(self, centro, ancho, largo, angulo, color):
        """Dibuja un rectángulo rotado alrededor de su centro"""
        puntos = []
        half_largo = largo / 2
        half_ancho = ancho / 2
        
        # Calcular las esquinas del rectángulo no rotado
        esquinas = [
        ...
        for x, y in esquinas:
            ...
            puntos.append((centro[0] + x_rot, centro[1] + y_rot))
        
        # Dibujar el rectángulo rotado
        pygame.draw.polygon(self.pantalla, color, puntos)

    def dibujar_robot(self):
        """Dibuja un robot circular con dos ruedas que rotan con la orientación"""
        centro = (int(self.robot.x), int(self.robot.y))
        radio = 20  # Radio del cuerpo principal del robot
        angulo = self.robot.orientacion

        # Dibujar cuerpo del robot (círculo)
        pygame.draw.circle(self.pantalla, NEGRITO, centro, radio)

        # Parámetros de las ruedas
        distancia_ruedas = 25  # Distancia desde el centro a las ruedas
        ancho_rueda = 8
        largo_rueda = 15

        # Calcular posiciones de las ruedas en función de la orientación
        # Rueda izquierda (90 grados a la izquierda de la orientación)
        angulo_rueda_izq = angulo + math.pi/2
        pos_rueda_izq = (
        centro[0] + distancia_ruedas * math.cos(angulo_rueda_izq),
        centro[1] + distancia_ruedas * math.sin(angulo_rueda_izq)
    )

        # Rueda derecha (90 grados a la derecha de la orientación)
        angulo_rueda_der = angulo - math.pi/2
        pos_rueda_der = (
        centro[0] + distancia_ruedas * math.cos(angulo_rueda_der),
        centro[1] + distancia_ruedas * math.sin(angulo_rueda_der)
    )

        # Función para dibujar una rueda rotada
        def dibujar_rueda_rotada(pos, ang):
            # Crear una superficie temporal para la rueda
            surf_rueda = pygame.Surface((largo_rueda, ancho_rueda), pygame.SRCALPHA)
            pygame.draw.rect(surf_rueda, NEGRO, (0, 0, largo_rueda, ancho_rueda))
        
            # Rotar la superficie
            surf_rotada = pygame.transform.rotate(surf_rueda, -ang * 180/math.pi)
        
            # Obtener el rectángulo de la superficie rotada y centrarlo en la posición
            rect = surf_rotada.get_rect(center=pos)
            self.pantalla.blit(surf_rotada, rect.topleft)

        # Dibujar ruedas rotadas
        dibujar_rueda_rotada(pos_rueda_izq, angulo)
        dibujar_rueda_rotada(pos_rueda_der, angulo)

    # Dibujar marca de orientación (línea frontal)
        punto_frente = (
        centro[0] + radio * math.cos(angulo),
        centro[1] + radio * math.sin(angulo))
        pygame.draw.line(self.pantalla, ROJO, centro, punto_frente, 2)
